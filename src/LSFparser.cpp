...
void LSFparser::buildDisplayLists(map<string, LSFnode*> &nodes,
	...
	if (nodes[rootNode]->isDisplayList) {
		...
		for (int unsigned i = 0; i < nodes[rootNode]->childPrimitives.size();
				i++) {
			...
			case rectangle: {
				...
				break;
			case triangle: {
				glNormal3f(primitive.normal.x, primitive.normal.y,
						primitive.normal.z);
				glBegin(GL_TRIANGLES);

				// Compute the height and width of the triangle
				LSFvertex p1, p2, p3;
				p1 = LSFvertex(primitive.attr["x1"], primitive.attr["y1"],
						primitive.attr["z1"]);
				p2 = LSFvertex(primitive.attr["x2"], primitive.attr["y2"],
						primitive.attr["z2"]);
				p3 = LSFvertex(primitive.attr["x3"], primitive.attr["y3"],
						primitive.attr["z3"]);
				...
				glVertex3d(primitive.attr["x1"], primitive.attr["y1"],
						primitive.attr["z1"]);

				glTexCoord2d(primitive.uvCoords[1].x * sFactor,
						primitive.uvCoords[1].y * tFactor);
				glVertex3d(primitive.attr["x2"], primitive.attr["y2"],
						primitive.attr["z2"]);

				glTexCoord2d(primitive.uvCoords[2].x * sFactor,
						primitive.uvCoords[2].y * tFactor);
				glVertex3d(primitive.attr["x3"], primitive.attr["y3"],
						primitive.attr["z3"]);
				glEnd();
			}
				break;
			case cylinder: {
				...
				gluCylinder(a, primitive.attr["base"], primitive.attr["top"],
						primitive.attr["height"], primitive.attr["slices"],
						primitive.attr["stacks"]);
			}
				break;
			case sphere: {
				GLUquadric *a = gluNewQuadric();
				gluQuadricNormals(a, GL_SMOOTH);
				gluQuadricTexture(a, GL_TRUE);
				gluSphere(a, primitive.attr["radius"], primitive.attr["slices"],
						primitive.attr["stacks"]);
			}
				break;
			case torus: {

				glutSolidTorus(primitive.attr["inner"], primitive.attr["outer"],
						primitive.attr["slices"], primitive.attr["loops"]);
			}
				break;
			}
	...
	for (int unsigned i = 0; i < nodes[rootNode]->childNoderefs.size(); i++) {
		buildDisplayLists(nodes, nodes[rootNode]->childNoderefs[i], appearances,
				appearancesStack,nodes[rootNode]->isDisplayList);
	}
	...
	if (nodes[rootNode]->isDisplayList){
		glEndList();
		// if is inside a display list
		if(enabledDisplayList){
			cout << ": " << rootNode << " - " << nodes[rootNode]->displayList << endl;
