...
void LSFprimitive::drawBooster(int way, GLfloat *texturespoints){
	if(way == 1 || way == -1){
	GLfloat BoosterR[16][3] = {
			{15.5, -0.1, 1.7*way}, {16.0, -0.1, 1.7*way},  {20.0, 0.0, 1.7*way},  {20.7, 0.0, 1.7*way},
			{15.7, -0.4, 1.7*way}, {16.0, -0.4, 1.7*way}, {20.0, -0.3, 1.7*way}, {20.4, -0.3, 1.7*way},
			{15.9, -0.7, 1.7*way}, {16.0, -0.7, 1.7*way}, {20.0, -0.6, 1.7*way}, {20.4, -0.6, 1.7*way},
			{16.0, -0.8, 1.8*way}, {16.0, -0.8, 1.8*way}, {20.0, -0.7, 1.8*way}, {20.7, -0.7, 1.8*way}};
	createEvaluator(&BoosterR[0][0], NULL, texturespoints);
	GLfloat BoosterRI[16][3] = {
			{16.0, -0.8, 1.8*way}, {16.0, -0.8, 1.8*way}, {20.0, -0.7, 1.8*way}, {20.7, -0.7, 1.8*way},
			{15.9, -0.7, 1.7*way}, {16.0, -0.7, 1.7*way}, {20.0, -0.6, 1.7*way}, {20.4, -0.6, 1.7*way},
			{15.7, -0.4, 1.7*way}, {16.0, -0.4, 1.7*way}, {20.0, -0.3, 1.7*way}, {20.4, -0.3, 1.7*way},
			{15.5, -0.1, 1.7*way}, {16.0, -0.1, 1.7*way},  {20.0, 0.0, 1.7*way},  {20.7, 0.0, 1.7*way}};
	createEvaluator(&BoosterRI[0][0], NULL, texturespoints);

	GLfloat BoosterB[16][3] = {
			{16.0, -0.8, 1.8*way}, {16.0, -0.8, 1.8*way}, {20.0, -0.7, 1.8*way}, {20.7, -0.7, 1.8*way},
			{16.0, -0.9, 1.9*way}, {16.0, -0.9, 1.9*way}, {20.0, -0.9, 2.0*way}, {20.4, -0.9, 1.9*way},
			{16.0, -0.9, 3.0*way}, {16.0, -0.9, 3.0*way}, {20.0, -0.9, 3.0*way}, {20.4, -0.9, 3.0*way},
			{16.0, -1.0, 3.0*way}, {16.0, -1.0, 3.0*way}, {20.0, -0.9, 3.0*way}, {20.7, -0.8, 3.1*way}};
	createEvaluator(&BoosterB[0][0], NULL, texturespoints);
	GLfloat BoosterBI[16][3] = {
			{16.0, -1.0, 3.0*way}, {16.0, -1.0, 3.0*way}, {20.0, -0.9, 3.0*way}, {20.7, -0.8, 3.1*way},
			{16.0, -0.9, 3.0*way}, {16.0, -0.9, 3.0*way}, {20.0, -0.9, 3.0*way}, {20.4, -0.9, 3.0*way},
			{16.0, -0.9, 1.9*way}, {16.0, -0.9, 1.9*way}, {20.0, -0.9, 2.0*way}, {20.4, -0.9, 1.9*way},
			{16.0, -0.8, 1.8*way}, {16.0, -0.8, 1.8*way}, {20.0, -0.7, 1.8*way}, {20.7, -0.7, 1.8*way}};
	createEvaluator(&BoosterBI[0][0], NULL, texturespoints);

	GLfloat BoosterL[16][3] = {
			{16.0, -1.0, 3.0*way}, {16.0, -1.0, 3.0*way}, {20.0, -0.9, 3.0*way}, {20.7, -0.8, 3.1*way},
			{15.9, -0.9, 3.1*way}, {16.0, -0.9, 3.1*way}, {20.0, -0.8, 3.1*way}, {20.4, -0.5, 3.1*way},
			{15.7, -0.7, 3.1*way}, {16.0, -0.7, 3.1*way}, {20.0, -0.5, 3.1*way}, {20.4, -0.4, 3.1*way},
			{15.5, -0.4, 3.1*way}, {16.0, -0.4, 3.1*way},  {20.0, -0.2, 3.1*way},  {20.7, -0.1, 3.1*way}};
	createEvaluator(&BoosterL[0][0], NULL, texturespoints);
	GLfloat BoosterLI[16][3] = {
			{15.5, -0.4, 3.1*way}, {16.0, -0.4, 3.1*way},  {20.0, -0.2, 3.1*way},  {20.7, -0.1, 3.1*way},
			{15.7, -0.7, 3.1*way}, {16.0, -0.7, 3.1*way}, {20.0, -0.5, 3.1*way}, {20.4, -0.4, 3.1*way},
			{15.9, -0.9, 3.1*way}, {16.0, -0.9, 3.1*way}, {20.0, -0.8, 3.1*way}, {20.4, -0.5, 3.1*way},
			{16.0, -1.0, 3.0*way}, {16.0, -1.0, 3.0*way}, {20.0, -0.9, 3.0*way}, {20.7, -0.8, 3.1*way}};
	createEvaluator(&BoosterLI[0][0], NULL, texturespoints);
	}
	if(way == 1){
	GLfloat BoosterFront[16][3] = {
			{16.0, -0.8, 1.8*way}, {16.0, -0.9, 1.9*way}, {16.0, -0.9, 2.8*way}, {16.0, -1.0, 3.0*way},
			{16.0, -0.7, 1.7*way}, {16.0, -0.8, 1.9*way}, {16.0, -0.8, 2.8*way}, {16.0, -0.9, 3.1*way},
			{16.0, -0.4, 1.7*way}, {16.0, -0.3, 1.9*way}, {16.0, -0.4, 2.8*way}, {16.0, -0.7, 3.1*way},
			{16.0, -0.1, 1.7*way}, {16.0, -0.2, 1.9*way},  {16.0, -0.3, 2.8*way},  {16.0, -0.4, 3.1*way}};
	createEvaluator(&BoosterFront[0][0], NULL, texturespoints);

	GLfloat BoosterBack[16][3] = {
			{20.3, -0.8, 3.1*way}, {20.3, -0.9, 3.0*way}, {20.3, -0.9, 2.0*way}, {20.3, -0.7, 1.8*way},
			{20.0, -0.5, 3.1*way}, {20.0, -0.5, 3.0*way}, {20.0, -0.5, 2.0*way}, {20.0, -0.6, 1.7*way},
			{20.0, -0.4, 3.1*way}, {20.0, -0.4, 3.0*way}, {20.0, -0.4, 2.0*way}, {20.0, -0.4, 1.7*way},
			{20.4, -0.15, 3.1*way}, {20.4, -0.1, 3.0*way},  {20.4, 0.0, 2.0*way}, {20.4, 0.0, 1.7*way}};
	createEvaluator(&BoosterBack[0][0], NULL, texturespoints);
	}
	if(way == -1){
	GLfloat BoosterFront[16][3] = {
			{16.0, -0.1, -1.7}, {16.0, -0.2, -1.9},  {16.0, -0.3, -2.8},  {16.0, -0.4, -3.1},
			{16.0, -0.4, -1.7}, {16.0, -0.3, -1.9}, {16.0, -0.4, -2.8}, {16.0, -0.7, -3.1},
			{16.0, -0.7, -1.7}, {16.0, -0.8, -1.9}, {16.0, -0.8, -2.8}, {16.0, -0.9, -3.1},
			{16.0, -0.8, -1.8}, {16.0, -0.9, -1.9}, {16.0, -0.9, -2.8}, {16.0, -1.0, -3.0}};
	createEvaluator(&BoosterFront[0][0], NULL, texturespoints);

	GLfloat BoosterBack[16][3] = {
			{20.4, -0.15, -3.1}, {20.4, -0.1, -3.0},  {20.4, 0.0, -2.0},  {20.4, 0.0, -1.7},
			{20.0, -0.4, -3.1}, {20.0, -0.4, -3.0}, {20.0, -0.4, -2.0}, {20.0, -0.4, -1.7},
			{20.0, -0.5, -3.1}, {20.0, -0.5, -3.0}, {20.0, -0.5, -2.0}, {20.0, -0.6, -1.7},
			{20.3, -0.8, -3.1}, {20.3, -0.9, -3.0}, {20.3, -0.9, -2.0}, {20.3, -0.7, -1.8}};
	createEvaluator(&BoosterBack[0][0], NULL, texturespoints);
	}
}

void LSFprimitive::drawBracket(float x, float y, float z, GLfloat *texturespoints){
	...
	createEvaluator(&WheelsBracketBack[0][0], NULL, texturespoints);
	GLfloat WheelsBracketFront[16][3] = {
			{0.0+x, -1.9+y, 0.0+z}, {-0.2+x, -1.9+y, 0.05+z}, {-0.2+x, -1.9+y, 0.25+z}, {0.0+x, -1.9+y, 0.3+z},
			{0.0+x, -1.1+y, 0.0+z}, {-0.2+x, -1.1+y, 0.05+z}, {-0.2+x, -1.1+y, 0.25+z}, {0.0+x, -1.1+y, 0.3+z},
			{0.0+x, -0.5+y, 0.0+z}, {-0.2+x, -0.5+y, 0.05+z}, {-0.2+x, -0.5+y, 0.25+z}, {0.0+x, -0.5+y, 0.3+z},
			{0.0+x, 0.0+y, 0.0+z},  {-0.2+x, 0.0+y, 0.05+z},  {-0.2+x, 0.0+y, 0.25+z},  {0.0+x, 0.0+y, 0.3+z}};
	createEvaluator(&WheelsBracketFront[0][0], NULL, texturespoints);
	...
	createEvaluator(&WheelsBracketBottom[0][0], NULL, texturespoints);
}

void LSFprimitive::drawWheel(float x, float y, float z, GLfloat *texturespoints){
	...
	createEvaluator(&WheelFrontR[0][0], NULL, texturespoints);
	...
	createEvaluator(&WheelBackR[0][0], NULL, texturespoints);
	...
	createEvaluator(&WheelR[0][0], NULL, texturespoints);
	GLfloat WheelL[16][3] = {
	...
	createEvaluator(&WheelL[0][0], NULL, texturespoints);
}
	...
	switch(type){
		...
		case vehicle:{
			...
			GLfloat topFrontBodypoints[16][3] = {
					{0.5, -0.5, 0.0}, {1.2, -0.3, 0.2},  {1.9, 0.0, 0.3},  {3.0, 0.0, 0.6},
					{0.5, -0.5, 0.0}, {1.2, 0.0, 0.1},  {1.9, 0.6, 0.2},  {3.0, 1.1, 0.4},
					{0.5, -0.5, 0.0}, {1.2, 0.0, -0.1}, {1.9, 0.6, -0.2}, {3.0, 1.1, -0.4},
					{0.5, -0.5, 0.0}, {1.2, -0.3, -0.2}, {1.9, 0.0, -0.3}, {3.0, 0.0, -0.6}};
			...
			GLfloat bottomFrontBodypoints[16][3] = {
					{0.5, -0.5, 0.0}, {1.2, 0.0, -0.2}, {1.9, 0.0, -0.3}, {3.0, 0.0, -0.6},
					{0.5, -0.5, 0.0}, {1.2, -0.6, -0.1}, {1.9, -0.4, -0.2}, {3.0, -0.5, -0.4},
					{0.5, -0.5, 0.0}, {1.2, -0.6, 0.1},  {1.9, -0.4, 0.2},  {3.0, -0.5, 0.4},
					{0.5, -0.5, 0.0}, {1.2, 0.0, 0.2},  {1.9, 0.0, 0.3},  {3.0, 0.0, 0.6}};
			...
			GLfloat topFrontWings[16][3] = {
					...
					{8.5, 0.1, 0.4},  {10.0, 0.2, 0.4},   {11.5, 0.2, 0.4},   {13.0, 0.3, 0.4},
					{8.5, 0.1, -0.4}, {10.0, 0.2, -0.4},  {11.5, 0.2, -0.4},  {13.0, 0.3, -0.4},
					{8.5, 0.0, -0.7}, {10.0, -0.1, -1.1}, {11.5, -0.1, -1.6}, {13.0, -0.2, -2.1}};
			...
			GLfloat topCenterWings[16][3] = {
					...
					{13.0, 0.3, 0.4},   {14.5, 0.3, 0.4},   {16.0, 0.3, 0.4},   {17.5, 0.3, 0.4},
					{13.0, 0.3, -0.4},  {14.5, 0.3, -0.4},  {16.0, 0.3, -0.4},  {17.5, 0.3, -0.4},
					{13.0, -0.2, -2.1}, {14.5, -0.3, -2.6}, {16.0, -0.5, -3.3}, {17.5, -0.5, -4.4}};
			createEvaluator(&topCenterWings[0][0], NULL, &texturespoints[0][0]);

			GLfloat topBackWings[16][3] = {
					{17.5, -0.5, 4.4},  {19.0, -0.5, 5.6},  {20.0, -0.5, 6.0},  {20.5, -0.5, 5.5},
					{17.5, 0.3, 0.4},   {19.0, 0.3, 0.4},   {20.0, 0.3, 0.4},   {20.5, 0.3, 0.4},
					{17.5, 0.3, -0.4},  {19.0, 0.3, -0.4},  {20.0, 0.3, -0.4},  {20.5, 0.3, -0.4},
					{17.5, -0.5, -4.4}, {19.0, -0.5, -5.6}, {20.0, -0.5, -6.0}, {20.5, -0.5, -5.5}};
			...
			GLfloat bottomBackWings[16][3] = {
					{17.5, -0.5, -4.4}, {19.0, -0.5, -5.6}, {20.0, -0.5, -6.0}, {20.5, -0.5, -5.5},
					{17.5, -0.1, -0.4}, {19.0, 0.0, -0.4},  {20.0, 0.1, -0.4},  {20.5, 0.3, -0.4},
					{17.5, -0.1, 0.4},  {19.0, 0.0, 0.4},   {20.0, 0.1, 0.4},   {20.5, 0.3, 0.4},
					{17.5, -0.5, 4.4},  {19.0, -0.5, 5.6},  {20.0, -0.5, 6.0},  {20.5, -0.5, 5.5}};
			createEvaluator(&bottomBackWings[0][0], NULL, &texturespoints[0][0]);

			GLfloat frontVerticalWings[16][3] = {
					{18.0, 1.0, 0.0},  {19.5, 0.9, 0.2},  {20.7, 0.9, 0.2},  {22.3, 0.9, 0.2},
			...
			GLfloat backVerticalWings[16][3] = {
					{22.3, 1.0, 0.2},  {22.9, 1.0, 0.15},  {23.0, 1.0, 0.1},  {23.4, 1.0, 0.0},
					{22.3, 3.8, 0.2},  {22.9, 3.8, 0.15},  {23.0, 3.5, 0.1},  {23.4, 1.0, 0.0},
					{22.3, 3.8, -0.2}, {22.9, 3.8, -0.15}, {23.0, 3.5, -0.1}, {23.4, 1.0, 0.0},
					{22.3, 1.0, -0.2}, {22.9, 1.0, -0.15}, {23.0, 1.0, -0.1}, {23.4, 1.0, 0.0}};
			createEvaluator(&backVerticalWings[0][0], NULL, &texturespoints[0][0]);

			//boosters
			drawBooster(1, &texturespoints[0][0]);
			drawBooster(-1, &texturespoints[0][0]);

			//wheelsBrackets
			drawBracket(8, -0.1, -0.15, &texturespoints[0][0]);
			drawBracket(17, -0.1, -1.7, &texturespoints[0][0]);
			drawBracket(17, -0.1, 1.4, &texturespoints[0][0]);

			//wheels
			drawWheel(8.0, -1.75, -0.15, &texturespoints[0][0]);
			drawWheel(8.0, -1.75, 0.35, &texturespoints[0][0]);

			drawWheel(17.0, -1.75, -1.7, &texturespoints[0][0]);
			drawWheel(17.0, -1.75, -1.2, &texturespoints[0][0]);

			drawWheel(17.0, -1.75, 1.4, &texturespoints[0][0]);
			drawWheel(17.0, -1.75, 1.9, &texturespoints[0][0]);
